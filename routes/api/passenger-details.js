...
router.get('/getPassenger', (req, res) => {
    // userCredentials.signedIn &&
    PassengerModel.find({userEmail: userCredentials.email})
...
router.post('/addPassenger', (req, res) => {
    ...
    // userCredentials.signedIn &&
    passenger.save().then((item) => res.status(201).json(item), (err) => res.status(400).json(err));

});

router.put('/editPassenger/:id', (req, res) => {
    // userCredentials.signedIn &&
    PassengerModel.findById(req.params.id)
    .then(passenger => {
